...
public class HibernateCommonDAO<T> extends HibernateDaoSupport {
	...
	// Logger.getLogger(HibernateCommonDAO.class);
	...
	public List find(final Criterion[] criterions, final Order[] orders, final int limit) {
	...
	public List<T> find(final List<Criterion> temp, int limit) {
		Criterion[] criterions = new Criterion[temp.size()];
		temp.toArray(criterions);
		return this.find(criterions, null, limit);
	}

	public List find(final List<Criterion> temp, final Order[] orders) {
		Criterion[] criterions = new Criterion[temp.size()];
		temp.toArray(criterions);
		return this.find(criterions, orders, -1);
	}
	...
	private PaginationSupport find(final Class<?> entity, final Criterion[] criterions, final Order[] orders, final int page,
	...
	private PaginationSupport find(final Class<?> entity, final Criterion[] criterions, final Order[] orders, final String joinTarget,
			final ArrayList joinQueryList, final int page, final int rowPerPage) {
		return (PaginationSupport) getHibernateTemplate().execute(new HibernateCallback() {
			@Override
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				Criteria criteria = session.createCriteria(entity);
